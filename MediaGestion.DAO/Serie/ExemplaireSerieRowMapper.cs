...
{
    public class ExemplaireSerieRowMapper : RowMapper<Exemplaire>
    {
        /// <summary>
        ...
        public override Exemplaire ExtraireDonneesReader(System.Data.SqlClient.SqlDataReader pReader)
        {
            ...
            try
            {
                ...
                serie.Code = pReader.GetGuid(pReader.GetOrdinal("codeSerie"));

                if (!pReader.IsDBNull(pReader.GetOrdinal("jaquette")))
                {
                    serie.Photo = pReader.GetString(pReader.GetOrdinal("jaquette"));
                }

                serie.LeGenre = new Genre(pReader.GetString(pReader.GetOrdinal("codeGenre")), pReader.GetString(pReader.GetOrdinal("libelleGenre")), pReader.GetByte(pReader.GetOrdinal("media")));
