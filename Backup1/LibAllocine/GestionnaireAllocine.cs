...
namespace LibAllocine
{
    public class GestionnaireAllocine
    {
        ...
        public ListeFichesFilmsAllocine RechercherFilm(string critere)
        {
            ...
            try
            {
                ...
                string param = String.Format(Properties.Settings.Default.ParamsRechercheAllocine, critere.Replace(" ", "+"), sed);
        ...
        /// </summary>
        /// <param name="critere"></param>
        public FicheFilmAllocine ObtenirFicheFilm(string codeFilm)
        {
            ...
            try
            {
                ...
                // Construction de l'url de recherche
                //string url = string.Format(Properties.Settings.Default.UrlInfosFilmAllocine, codeFilm);

                string url = "http://api.allocine.fr/rest/v3/movie";

                string sed = String.Format("{0:yyyyMMdd}", DateTime.Now);
                string param = String.Format(Properties.Settings.Default.ParamsInfosFilmAllocine, codeFilm, sed);
        ...
        private void DownloadJaquette(FicheFilmAllocine film) {

            try
            {
                Log.MonitoringLogger().Info(KS_NOM_MODULE + "Début DownloadJaquette");

                string nomJaquette = film.Titre + ".jpg";

                nomJaquette = nomJaquette.Replace(":", "");
                nomJaquette = nomJaquette.Replace("?", "");
                nomJaquette = nomJaquette.Replace("/", "");
                ...
                if (film.LaJaquette != null)
                {
                    Uri adress = new Uri(film.LaJaquette.Url);

                    webClient.DownloadFile(adress, cheminFichier);
                    film.NomJaquette = nomJaquette;
                }
                else
                {
